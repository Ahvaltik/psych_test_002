...
function setup() {
    ...
    arial = loadFont('//db.onlinewebfonts.com/t/8d223b3ad8d4819e9dcf22757e4cc2c4.otf');
    ...
    inpAlias.position(WIDTH / 2 - 100, HEIGHT / 2);
    inpAlias.size(200, 25);

    var inpAge = createInput('age');
    inpAge.position(WIDTH / 2 - 100, HEIGHT / 2 + 100);
    inpAge.size(200, 25);
    ...
    this.tick = function () {
        if (!this.stopped) {
            if (this.time < FIXATION_TIME) {
                ...
                s = new Primer(primers[testProgress][0]);
            }
                ...
                this.waiting = true;
                s = new Target(targets[testProgress]);
